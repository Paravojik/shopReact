...
const List=({addToCart})=>{
    ...
    const filteredItems=useMemo(()=>{
        return products.filter(item=>item.name.toLowerCase().includes(filterText.toLowerCase()))
    },[products,filterText]
    )
    return(
        <div className="List">
            <h2>Product List</h2>
            <input placeholder='Search by name' className='List__filter' type="text" value={filterText} onChange={e=>setFilterText(e.target.value)}/>
